...
Original file is located at
    https://colab.research.google.com/drive/1v-ltywOGLvUgqDZ0Edwke-6zoG3G0lW-
"""
...
# 需要根据决策变量的上下边界来确定
def getEncodedLength(delta=0.5, boundarylist=[]):
    # 每个变量的编码长度
    lengths = []
...
def getFitnessValue(func, chromosomesdecoded):
    ...
    #     print(chromosomesdecoded[i,0],chromosomesdecoded[i,1])
    for i in range(population):
        print('window : ', chromosomesdecoded[i][0])
        print('rolling : ', chromosomesdecoded[i][1]/2048)
        get = []
        ################## LOF rolling 輸出次數 #############
        data = pd.read_csv('/content/drive/MyDrive/LOF/LOF/INV3.csv', encoding='ISO-8859-1')
        df = pd.Series(data.mark).rolling(
            window=int(chromosomesdecoded[i][0])).mean()
        data['rolling'] = df
        data.to_csv('/content/drive/MyDrive/LOF/LOF/newINV3.csv', index=None)

        data = pd.read_csv('/content/drive/MyDrive/LOF/LOF/newINV3.csv',
                           encoding='ISO-8859-1')
        data.fillna(value=0, inplace=True)
        number = 0
        for k in range(len(data.mark)):
            if data['rolling'][k] > (chromosomesdecoded[i][1]/2048):
                number = number+1
        ...
        ################## temp rolling 輸出次數 #############
        data = pd.read_csv('/content/drive/MyDrive/LOF/temp/INV3.csv',
                           encoding='ISO-8859-1')
        df = pd.Series(data.mark).rolling(
            window=int(chromosomesdecoded[i][0])).mean()
        data['rolling'] = df
        data.to_csv('/content/drive/MyDrive/LOF/temp/newINV3.csv', index=None)

        data = pd.read_csv('/content/drive/MyDrive/LOF/temp/newINV3.csv',
                           encoding='ISO-8859-1')
        data.fillna(value=0, inplace=True)
        number1 = 0
        for j in range(len(data.mark)):
            if data['rolling'][j] > (chromosomesdecoded[i][1]/2048):
                number1 = number1+1

        print('number2:', number1)
        get.append(number1)
        ################## temp rolling 輸出次數 #############
        fitnessvalues[i, 0] = func(get)
...
def crossover(population, Pc=0.8):
    ...
    while len(index) > 0:
        ...
        crossoverPoint1 = random.sample(range(1, n), 1)
        crossoverPoint = crossoverPoint[0]
        crossoverPoint1 = crossoverPoint1[0]

        if crossoverPoint1 < crossoverPoint:
          temp = crossoverPoint
          crossoverPoint = crossoverPoint1
        ...
        # # two-siingle-point
        updatepopulation[a, 0:crossoverPoint] = population[a, 0:crossoverPoint]
        updatepopulation[a, crossoverPoint1:] = population[a, crossoverPoint1:]
        updatepopulation[a, crossoverPoint:crossoverPoint1] = population[b, crossoverPoint:crossoverPoint1]
        
        updatepopulation[b, 0:crossoverPoint] = population[b, 0:crossoverPoint]
        updatepopulation[b, crossoverPoint1:] = population[b, crossoverPoint1:]
        updatepopulation[b, crossoverPoint:crossoverPoint1] = population[a, crossoverPoint:crossoverPoint1]
...
def main(max_iter=500):
    ...
    for iteration in range(max_iter):
        ...
        fitnessvalues, cum_individual_proba = getFitnessValue(
            fitnessFunction(), final_decoded)
        # 搜索每次迭代的最优解，以及最优解对应的目标函数的取值
        optimalValues.append(np.min(list(fitnessvalues)))
        index = np.where(fitnessvalues == np.min(list(fitnessvalues)))
...
# ################## LOF標記 #############
# print('1111111')
# data=pd.read_csv('D:/LOF test/LOFresult/INV14.csv',encoding='ISO-8859-1')
# data.fillna(method='ffill',inplace=True)


# m=[]
# for i in range(len(data.Temp_Sink)):
#     if data.distance[i]>(2.07764482+ 0.6290870616500494):
#         m.append(1)
#     else:
#         m.append(0)
...
# path='G:/GA test/LOF/'
# data['mark']=m
# data.to_csv(path+'INV14.csv',index=None)
# ################## LOF標記 #############

# ################## temp標記 #############
# print('2222222')
# data=pd.read_csv('G:/use_data/two_year/INV/INV14.csv',encoding='ISO-8859-1')
# data.fillna(method='ffill',inplace=True)


# m=[]
# for i in range(len(data.Temp_Sink)):
#     if data.Temp_Sink[i]>60:
#         m.append(1)
#     else:
#         m.append(0)
...
for i in range(6):
    ...
    s1.append(solution[0])
    s2.append(solution[1])
    print('最優目標函數值:', value)
...
print('平均 : ', mean)
st_dev = np.std(average)
print('標準差 :  ', st_dev)
print('最佳解 :  ', min(average))